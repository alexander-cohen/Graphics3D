...
arrayvec *spherePoints(double x, double y, double z, double radius, int steps) {
    ...
    arrayvec *pts = av_create(steps * (steps + 1), sizeof(Vec3));
    int i, j, idx = 0;
    double theta, phi, rsp;
    Vec3 pole0 = (Vec3) {x, y, z+radius},
            pole1 = (Vec3) {x, y, z-radius};
    for(i = 0; i < steps; i++) {
        for(j = 0; j < steps; j++) {
            printf("i=%d, j=%d\n", i, j);
            if(j == 0) {
            ...
            if(j == steps/2) { // ensure pole consistency
                av_append(pts, &pole1, false);
                continue;
            }
    ...
    for(i = steps; i < steps * steps; i += steps) {
...
arrayvec *sphereTris(int steps) {  // returns arrayvec of ints corresponding to point indices
    ...
    for(i = 0; i < steps; i++) {
        for(j = 0; j < steps; j++, idx++) { // covering the easy cases
            iN = idx + 1;
            iTN = idx + steps;
            iT = iTN++;
